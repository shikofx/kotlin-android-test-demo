...
* [x] **Задачи по CI/CD (GitHub Actions):**
  ...
  * [ ] Оптимизировать пайплайн для параллельного запуска задач после подготовки зависимостей.
* [ ] **Финализация:**
  * [ ] Ветка `feature/3-integration-tests` готова к слиянию в `main`.
...
## Этап 6: Рефакторинг бизнес-логики (31.10 - 03.11)
...
## Этап 7: Рефакторинг и тестирование сетевого слоя (04.11 - 05.11)

**Цель:** Сделать класс `Network.java` тестируемым и покрыть его интеграционными тестами с `MockWebServer`.

* [ ] **Ветка:** `feature/7-network-refactoring-and-tests` (создается из `main`)
* [ ] **Задачи по коду:**
  * [ ] Провести рефакторинг `Network.java`: избавиться от статики, внедрять базовый URL через конструктор.
  * [ ] Написать интеграционные тесты для `Network.java` с `MockWebServer`, покрывающие успешные сценарии, ошибки и проверку заголовков.
* [ ] **Финализация:**
  * [ ] Выполнить слияние ветки `feature/7-network-refactoring-and-tests` в `main`.

## Этап 8: Рефакторинг UI-слоя (06.11 - 07.11)
...
## Этап 5: Современный подход с Kaspresso (23.10 - 30.10) - _Перенесен после рефакторинга_
...
## Этап 9: Финальный рефакторинг и оптимизация CI (10.11 - 15.11)

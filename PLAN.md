...
| `feature/5-kakao` | **Улучшение читаемости:** Тесты переписаны с использованием DSL-обертки **Kakao**, внедрен паттерн Page Object. | `Kakao`, `Page Object` |
...
| `feature/8-integration-refactoring` | **Рефакторинг слоя данных:** Исправление кода, связанного с базой данных, под защитой интеграционных тестов. | `Java`, `Room` |
| `feature/9-ui-refactoring` | **Рефакторинг UI-слоя:** Исправление кода UI-компонентов под защитой Espresso-тестов. | `Java`, `Espresso` |
| `feature/10-kotlin-final-refactoring` | **Финальный рефакторинг:** Проведен масштабный рефакторинг исходного кода приложения под защитой Kaspresso-тестов. | `Kotlin`, `ViewBinding`, `Gradle KTS` |
...
*   [ ] **Задачи по CI/CD:**
    ...
    *   [ ] Настроить запуск интеграционных тестов при создании **`pull_request` в `develop`** (или при `push` в `feature/*`).
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/3-integration-tests` в `develop`.

## Этап 4: UI-тестирование (Espresso) (13.10 - 18.10)

**Цель:** Написать первые UI-тесты и интегрировать их результаты в существующую CI/CD и Allure-инфраструктуру.

*   [ ] **Ветка:** `feature/4-espresso-initial` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
*   [ ] **Задачи по CI/CD (GitHub Actions):**
    ...
    *   [ ] Настроить запуск UI-тестов при создании **`pull_request` в `develop`**.
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/4-espresso-initial` в `develop`.

## Этап 5: Улучшение читаемости тестов с Kakao (19.10 - 24.10)

**Цель:** Улучшить поддерживаемость тестов с помощью DSL Kakao и паттерна Page Object.

*   [ ] **Ветка:** `feature/5-kakao` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/5-kakao` в `develop`.

## Этап 6: Современный подход с Kaspresso (25.10 - 05.11)

**Цель:** Освоить фреймворк Kaspresso, покрыть сложные сценарии, интегрироваться с облачными фермами.

*   [ ] **Ветка:** `feature/6-kaspresso` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/6-kaspresso` в `develop`.

## Этап 7: Рефакторинг бизнес-логики (06.11 - 09.11)
...
*   [ ] **Ветка:** `feature/7-logic-refactoring` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/7-logic-refactoring` в `develop`.

## Этап 8: Рефакторинг слоя данных (10.11 - 13.11)

**Цель:** Улучшить код, связанный с базой данных и репозиториями, под защитой написанных интеграционных тестов.

*   [ ] **Ветка:** `feature/8-integration-refactoring` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
*   [ ] **Финализация:**
    *   [ ] Выполнить слияние ветки `feature/8-integration-refactoring` в `develop`.

## Этап 9: Рефакторинг UI-слоя (14.11 - 17.11)

**Цель:** Улучшить код UI-компонентов (`Fragments`, `Activities`), который теперь покрыт Espresso-тестами.

*   [ ] **Ветка:** `feature/9-ui-refactoring` (создается из `develop`)
*   [ ] **Задачи по коду:**
...
## Этап 10: Финальный рефакторинг и оптимизация CI (18.11 - 30.11)
...
*   [ ] **Ветка:** `feature/10-kotlin-final-refactoring` (создается из `develop`)

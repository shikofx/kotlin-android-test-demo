...
1.  **`stages` (Этапы):**
    ...
    Порядок, заданный в `stages`, работает следующим образом:
        1.  Все задачи (`jobs`), принадлежащие к первому этапу из списка, запускаются **параллельно**.
        2.  Следующий этап начнется только после того, как **все** задачи предыдущего этапа **успешно** завершатся.
        3.  Если хотя бы одна задача провалится, пайплайн останавливается.
    ...
    **Важно:** `stages` лишь определяет порядок. Пайплайн не будет создан, если в файле не определена хотя бы одна задача (`job`).
...
5.  **`image`:**
    ...
    *   **А что если `image` не указан нигде?**
        В этом случае Runner будет использовать образ, который был задан при его собственной настройке в файле `config.toml` (см. Урок 11). В нашем случае это `alpine:latest`. Это гарантирует, что у задачи всегда будет окружение для выполнения `script`-команд.
...
1.  **`default` (Настройки по умолчанию):**
    ...
    *   `tags: - docker`: Это фильтр. Он говорит: "Эту задачу может выполнить только тот Runner, у которого есть тег `docker`". Мы указывали этот тег при настройке Runner'а в Уроке 11.
...
3.  Он находит задачу `check_job` и начинает искать свободный Runner с тегом `docker`.
4.  Наш Runner на сервере "слышит" GitLab, принимает задачу и начинает работу.
5.  Поскольку у Runner'а `executor = "docker"`, он:
    a.  Скачивает образ `gitlab.t366.xyz:5050/shikofx/kotlin-android-test-demo/android-builder:latest`.
    b.  Запускает из этого образа новый контейнер.
    c.  Внутри этого контейнера выполняет команду `echo "Проверка пайплайна"`.
